import { TodaClientV2, WaitForHitchError } from "../../src/client/client.js";
...
import { RemoteNextRelayClient, LocalNextRelayClient } 
...
describe("create", async () => {
    ...
    it("should create a Twist with the correct properties", async () => {
        ...
        let toda = new TodaClientV2(new LocalInventoryClient("./files/" + uuid()));
...
describe("append", async () => {
    ...
    it("should hoist", async () => {
        let toda = new TodaClientV2(new LocalInventoryClient(`${__dirname}/files`));
...
describe("Stopping conditions", async () => {
    it("No data seeded: stops at specified poptop", async () => {
        ...
        try {
            ...
            assert.ok(!topRelay.requestLogs.includes(`GET /files/${top0.getHash()}.next.toda`));
    ...
    it("No data seeded: stops at fast twist before proceeding to poptop", async () => {
        ...
        try {
            ...
            assert.ok(!midRelay.requestLogs.includes(`GET /files/${mid1.getHash()}.next.toda`));
    ...
    it("When data already exists and there are no fast twists in the relay, will stopRelay at the most recently known loose twist", async () => {
        ...
        try {
            ...
            assert.ok(!topRelay.requestLogs.includes(`GET /files/${top2.getHash()}.next.toda`));
...
xdescribe("pull should include all required info", async () => {
    it("send to remote and back", async () => {
        ...
        let keyPair = await SECP256r1.generate();
        let toda = new TodaClientV2(new LocalInventoryClient(dir0));
        ...
        let toda2 = new TodaClientV2(new LocalInventoryClient(dir1));
...
describe("Multi-remote pull test", () => {
    ...
    it("Deep pull, no loose twists", async () => {
        ...
        const c = new TodaClientV2(new LocalInventoryClient("./files/" + uuid()), 
                                        "http://localhost:8090/files");
        ...
        const b = new TodaClientV2(new LocalInventoryClient("./files/" + uuid()), 
                                                            "http://localhost:8091/files");
        ...
        try {
            ...
            const aLine = await (new LocalNextRelayClient(a, a0.getHash())).get();

            // Sanity check
            assert.ok(!aLine.get(d2.getHash()));
            assert.ok(!aLine.get(c2.getHash()));
            assert.ok(!aLine.get(b2.getHash()));
