...
const { getSuccessor, generateShield, getFileOrHashPath } = require("../util");
 ...
 */
async function create(shield, req, tether, pk, cargo, config) {
    ...
    if (tether) {
        await setFastFields(tether, tb, config, shield);
    }
 ...
 * @returns <TwistBuilder> a TwistBuilder object that can be used to generate the Twist
 */
async function append(abject, shield, req, tether, pk, setterFn = () => {}, cargo, config) {
 ...
 */
async function setFastFields(tether, tb, config, shield) {
    ...
    if (!shield && !getFileOrHashPath(null, tether)) {
        let salt = new ByteArray(fs.readFileSync(config.salt));
    ...
    if (lastFastTwist) {
        let tetherUrl = await getTetherUrl(tb, config);
...
async function hoistLocal(lead, meetHash, path, pk, config) {
    ...
    let tetherUrl = tether.isTethered() ? await getTetherUrl(tether, config) : null;
    let tb = await append(tether, null, null, tetherUrl, pk, setterFn, null, config);
 ...
 * @param config <Object> A config object containing the local line and default line server paths
 */
async function hoist(abject, pk, config) {
    ...
    if (lastFastTwist) {
        ...
        return fs.existsSync(tetherUrl)
            ? hoistLocal(lastFastTwist, abject.getHash(), tetherUrl, pk, config)
